...
public class BookingToolsImpl implements BookingTools{
	...
	public Booking confirmBooking(Customer customer,ParkingSpot spot,VehicleType vehicleType,String vehicleNumber,boolean royalPark,boolean carPooled) {
		...
		bookingObj.setCarPooled(carPooled);
	...
	public boolean vacateParking(String  bookingId) {
		...
		//for demo purpose we re checking in mitues otherwise we will check fro 24 hours
		long timeInterval=(booking.getDateOfVacate().getTime()-booking.getDateOfBooking().getTime())/(60*1000)%60;
		...
		//if car is pooled further decrement the cost
		if(booking.isCarPooled()) {
			booking.setBookingCost(booking.getBookingCost()-20);
		}
