...
public class ParkingFloorToolsImpl implements ParkingFloorTools{
	...
	/* (non-Javadoc)
	 * releaseFromParkingSpot for both normal and royal parked vehicle
	 */
	public boolean releaseFromParkingSpot(ParkingFloor parkingFloor, int parkingSpotId) {
		ParkingSpot parkingSpot = null;
		...
		if(removeVehicleFromSpot && parkingSpot.getVehicleCounter()==0) {
			...
			if(parkingFloor.getFullyOccuParkingSpotsMap().containsKey(parkingSpot.getSpotId())) {
				if(royalFamilyFlag && parkingSpot.getSpotId() %5 ==1) {
					//changing the next spot if royal placed in first column 
					...
					////changing the previous spot if royal placed in last column 
					ParkingSpot parkingSpot2 = parkingFloor.getFullyOccuParkingSpotsMap().get(parkingSpot.getSpotId() - 1);
					parkingSpot2.setVehicleCounter(0);
					parkingSpot2.setVehicleType(null);
					parkingFloor.getFullyOccuParkingSpotsMap().remove(parkingSpot2.getSpotId());
	...
	/* 
	 * check for royal customer in corner and check next and previous for first and last column of rows respctively.
	 * 
	 * @return parking spot
	 */
	...
	/* 
	 *check for the random spot and also check the previous and next spot for royal and royal presence
	 *
	 *@return parking spot
	 */
	public ParkingSpot checkForRandomParkingSpot(Map<Integer, ParkingSpot> parkingMap, boolean royalFamilyFlag,
		...
		while (it.hasNext()) {
			Map.Entry<Integer, ParkingSpot> availableParkingobj = (Entry) it.next();
			if (royalFamilyFlag) {
				//check for random spot not in corner and also check the previous and next spot that should be empty ie available in available spot
